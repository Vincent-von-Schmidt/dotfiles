...
--         win = 0,
--     })
--     require("plugins.telescope.picker.launcher").picker()
-- end, { silent = true, noremap = true })
...
vim.keymap.set("n", "<leader>e", function()
	local tabs_open = {}
	for el, _ in ipairs(vim.api.nvim_list_tabpages()) do
		table.insert(tabs_open, "Vimux tab: " .. el)
	end
	vim.ui.select(tabs_open, { prompt = "Vimux-Tabs" }, function(item, idx)
		if item ~= nil then
			vim.cmd(idx .. "tabnext")
		end
	end)
